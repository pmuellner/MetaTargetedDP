...
def select_params(best_params, e):
    for params in best_params:
        if params["e"] == e:
            return params["lr"], params["reg_factor"]
    else:
...
if __name__ == "__main__":
    ...
    CLI.add_argument("--model", type=str, default="MetaMF", choices=["MetaMF", "NoMetaMF"])
    ...
    CLI.add_argument("--seed", nargs="+", type=int, default=[0])
    args = CLI.parse_args()
    ...
    regularization_factor = np.inf
    batch_size = 64
    with open("hypertuning_results/" + dataset + "/" + model_name + ".best_params", "rb") as f:
    ...
    if method == "baseline":
        ...
        epsilons = [3, 2, 1, 0.1]
        configs = list(cartesian_product(epsilons, betas))
    ...
    for s in seeds:
        ...
        for c in configs:
            e, b = c
            learning_rate, regularization_factor = select_params(best_params, e=e)
            print("==================================")
            print("Model: %s, Method: %s, Dataset: %s, Seed: %d" % (model_name, method, dataset, s))
            print("lr: %f, reg_factor: %f" % (learning_rate, regularization_factor))
            print("b: %f, e: %f" % (b, e))
            ...
            # model training
            hyperparameters = {"lr": learning_rate, "lambda": regularization_factor, "batch_size": batch_size, "n_epochs": 1000}
        ...
        file = open("results/" + dataset + "/" + model_name + "_s" + str(s) + "." + method, "wb")
